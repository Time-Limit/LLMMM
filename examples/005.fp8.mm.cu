...
void fp8_gemm_blockwise_quant_A_1x128__B_128x128__C_1x128__output_fp8(const __nv_fp8_e4m3* A,
                                                                      const float*         A_scale_transposed,
                                                                      const __nv_fp8_e4m3* B_transposed,
                                                                      const float*         B_scale_transposed,
                                                                      __nv_fp8_e4m3*       C,
                                                                      ...
                                                                      cudaStream_t         stream)
{
  ...
  dim3 grid(N / BLOCK_N, M / BLOCK_M);
  dim3 block(WARP_COUNT * 32);
  fp8_gemm_blockwise_quant_A_1x128__B_128x128__C_1x128__output_fp8<BLOCK_M, BLOCK_N, WARP_M, WARP_N, LOOP_K>
    <<<grid, block, 0, stream>>>(
...
int main()
{
  ...
  fp8_gemm_blockwise_quant_A_1x128__B_128x128__C_1x128__output_fp8(
    d_A_q, d_A_s, d_B_q, d_B_s, d_C_q, d_C_s, M, N, K, nullptr);
  CHECK_CUDA_ERROR();
  ...
  CHECK_CUDA_RETURN(cudaMemcpy(h_C_s.data(), d_C_s, sizeof(float) * h_C_s.size(), cudaMemcpyDefault));
  {
    ...
    printf(
      "check fp8 mm, max_abs_error = %10.8f, max_relative_error = %10.8f, avg_abs_error = %10.8f, avg_relative_error = %10.8f, base = %10.8f, deq = %10.8f, q = %10.8f, s = %10.8f, m = %5d, n = %5d\n",
      max_abs_error,
      max_abs_error / fabs(base),
      sum_abs_error / (M * N),
      sum_abs_error / sum_abs_value,
